$(document).ready(function() {
    //Ordina secondo la betwenness
    var pag = 1;
    var text = $("h2").text();
    var maxBet = $("h6").text();
    var maxInd = $("h5").text();
    
    $("#ord-bet").click(function() {
        $("#pag").hide();
        loadDataBet(text, pag, maxBet, maxInd);
        $("#ord").show();
        $("#ord-bet").hide();
        $("#ord-cit").show();
        $("#pag-ord").hide();
...
function loadDataBet(text, page, maxBet, maxInd) {
    $.ajax({
        type: "POST",
        url: "bet.php",
        data: text + "&page=" + page,
