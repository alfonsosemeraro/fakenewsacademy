$(document).ready(function() {
    //Ordina secondo i valori di clustering
    var pag = 1;
    var text = $("h2").text();
    var maxBet = $("h6").text();
    var maxInd = $("h5").text();
    
    $("#ord-clust").click(function() {
        $("#pag").hide();
        loadDataClust(text, pag, maxBet, maxInd);
        $("#ord").show();
        $("#ord-cit").show();
        $("#ord-bet").show();
        $("#ord-clust").hide();
...
function loadDataClust(text, page, maxBet, maxInd) {
    $.ajax({
        type: "POST",
        url: "clust.php",
        data: text + "&page=" + page,
