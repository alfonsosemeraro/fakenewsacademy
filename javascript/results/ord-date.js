$(document).ready(function() {
    //Ordina secondo l'anno di pubblicazione dell'articolo
    var pag = 1;
    var text = $("h2").text();
    var maxBet = $("h6").text();
    var maxInd = $("h5").text();
    ...
    $("#ord").click(function() {
        $("#pag").hide();
        loadDataOrd(text, pag, maxBet, maxInd);
        $("#ord").hide();
        $("#ord-cit").show();
        $("#pag-ord").show();
        $("#ord-bet").show();
...
function loadDataOrd(text, page, maxBet, maxInd) {
    $.ajax({
        type: "POST",
        url: "ordered.php",
        data: text + "&page=" + page,
