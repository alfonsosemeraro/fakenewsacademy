$(document).ready(function() {
    //Ordina secondo l'indegree dell'articolo
    var pag = 1;
    var text = $("h2").text();
    var maxBet = $("h6").text();
    var maxInd = $("h5").text();

    $("#ord-ind").click(function() {
        $("#pag").hide();
        loadDataInd(text, pag, maxBet, maxInd);
        $("#ord").show();
        $("#ord-cit").show();
        $("#ord-bet").show();
        $("#pag-ord").hide();
...
function loadDataInd(text, page, maxBet, maxInd) {
    $.ajax({
        type: "POST",
        url: "ind.php",
        data: text + "&page=" + page,
