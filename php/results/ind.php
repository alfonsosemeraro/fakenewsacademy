...
//Fa una echo degli articoli ordinati secondo l'indegree.
...
if (isset($_REQUEST["page"])) {
    ...
    if (isset($_REQUEST["search"])) {
        ...
        $query = $query . " ORDER by indegree DESC LIMIT $from, $max_results";
        $rows = $db->query($query);
    } else {
        $search = $_REQUEST["title"];
        $result = $db->query("SELECT * FROM papers WHERE title LIKE '%$search%'");
        $total_pages = ceil($result->rowCount() / $max_results); 
        $rows = $db->query("SELECT * FROM papers WHERE title LIKE '%$search%' OR abstract LIKE '%$search%' ORDER by indegree DESC LIMIT $from, $max_results");
    }
    if($rows->rowCount()>0) {
        foreach ($rows as $row) {
            $jsonarray[] = $row;
