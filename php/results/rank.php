...
//Fa una echo degli articoli ordinati secondo il pagerank.
...
if (isset($_REQUEST["page"])) {
    ...
    if (isset($_REQUEST["search"])) {
        ...
        $query = $query . " ORDER by pagerank DESC LIMIT $from, $max_results";
        $rows = $db->query($query);
    } else {
        $search = $_REQUEST["title"];
        $result = $db->query("SELECT * FROM papers WHERE title LIKE '%$search%'");
        $total_pages = ceil($result->rowCount() / $max_results); 
        $rows = $db->query("SELECT * FROM papers WHERE title LIKE '%$search%' OR abstract LIKE '%$search%' ORDER by pagerank DESC LIMIT $from, $max_results");
    }
    if($rows->rowCount()>0) {
        foreach ($rows as $row) {
            $jsonarray[] = $row;
